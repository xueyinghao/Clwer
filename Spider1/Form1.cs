...
namespace Spider1
{
    public partial class Form1 : Form
    {
        ...
        public static void GetUrl(string url)
        {
            ...
            if (url.Contains(".xml") || url.Contains(".rar"))
            {
                return;
            }
            //处理500错误
            client.Headers["User-Agent"] = "blah";
            try
            {
            ...
            {
                string str = url + ex.Message;
                writeErrLog(str);
            }
            string content = System.Text.Encoding.UTF8.GetString(page);
            #region     通过HttpWebRequest来获取(暂时不用)
            //string content = "";        //定义字符串，用来保存下载的html
            //int byteRead = 0;
            ...
            //    StreamReader resStreamReader = new StreamReader(reader, Encoding.UTF8);
            //    //分段，分批次获取网页源码
            //    char[] cbuffer = new char[1024];
            //    byteRead = resStreamReader.Read(cbuffer, 0, 256);
            //    while (byteRead != 0)
            //    {
            //        string strResp = new string(cbuffer, 0, byteRead);
            //        content = content + strResp;
            //        byteRead = resStreamReader.Read(cbuffer, 0, 256);
        ...
        //将获取到的URL写入到txt文本里面
        public static void writeUrlLog(string str)
        {
            writebytes = Encoding.UTF8.GetBytes(str + "\r\n\r\n");
            //之前的Save方法 Save(byte[] write)
            FileStream file = new FileStream("d:\\urlLog.txt", FileMode.Append, FileAccess.Write);
            file.Write(writebytes, 0, writebytes.Length);
            file.Dispose();
            file.Close();
            writebytes.Initialize();
        ...
        public static void writeErrLog(string str)
        {
            writebytes = Encoding.UTF8.GetBytes(str + "\r\n\r\n");
            //之前的Save方法 Save(byte[] write)
            FileStream file = new FileStream("d:\\ErrLog.txt", FileMode.Append, FileAccess.Write);
            file.Write(writebytes, 0, writebytes.Length);
            file.Dispose();
            file.Close();
            writebytes.Initialize();
        ...
        public static void OperateDB(string url)
         {
            ...
            conn.Open();
            string sql = "Insert into UrlIndex values('" + url + "')";
